...
fn main() {
    ...
    let sieve = EratosthenesSieve::new(r);

    let mut ans = 0;
    for i in 2..=r {
        let mul_num = r / i - (l - 1) / i;
        let mut factors = vec![];
        let mut t = i;
        while t != 1 {
            let p = sieve.get_least_prime_factor(t);
            factors.push(p);
            t /= p;
        }
        ...
        factors.dedup();
        if factors.len() == factor_num {
            let pair_num = mul_num.pow(2);
            if factor_num % 2 == 1 {
                ans += pair_num;
            } else {
                ans -= pair_num;
    ...
    ans -= 2 * (l.max(2)..=r).map(|i| r / i - (l - 1) / i).sum::<usize>() - (r - l.max(2) + 1);
