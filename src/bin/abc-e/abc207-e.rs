...
fn main() {
    ...
    // acc[i]: Aの長さiのprefixの総和
    let mut acc = vec![0; n + 1];
    for i in 0..n {
        acc[i + 1] = acc[i] + aa[i];
    }

    // dp[i][j]: Aの長さiのprefixをj個の数列に分割する方法の数
    let mut dp = vec![vec![Mint::new(0); n + 1]; n + 1];
    // Aの長さ0のprefixを0個に分割する方法の数は1通り
    dp[0][0] = Mint::new(1);
    // rems[j][k]: 総和をjで割るとk余るようなAのprefixをj-1個の数列に分割する方法の数
    // 許容されるprefixの長さの上限は順次更新され，次のループのカウンタ変数iの値と一致する
    let mut rems = vec![vec![Mint::new(0); n + 1]; n + 1];
    for i in 0..n {
        for j in 0..n {
            rems[j + 1][acc[i] % (j + 1)] += dp[i][j];
        }

        for j in 1..=n {
            dp[i + 1][j] += rems[j][acc[i + 1] % j];
        }
    }

    println!("{}", dp[n].iter().fold(Mint::new(0), |acc, &x| acc + x));
}
