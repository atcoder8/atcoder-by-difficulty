...
fn main() {
    ...
    let count = |digit: usize| -> usize {
        let mut cnt = 0;

        let mut evens = vec![0_usize; n];
        let mut odds = vec![0_usize; n];
        let mut stack: Vec<(Option<usize>, usize, bool)> = vec![(None, 0, false)];
        while let Some((par, cur, back)) = stack.pop() {
            if back {
                let mut sub_evens = vec![1];
                let mut sub_odds = vec![0];
                for &(next, weight) in &graph[cur] {
                    if Some(next) == par {
                        continue;
                    }

                    let mut even = evens[next];
                    let mut odd = odds[next];
                    if (weight >> digit) & 1 == 1 {
                ...
                evens[cur] = sub_evens.iter().sum();
                odds[cur] = sub_odds.iter().sum();

                for (&even, &odd) in sub_evens.iter().zip(&sub_odds) {
                    cnt += even * (odds[cur] - odd) + odd * (evens[cur] - even);
                }
    ...
    for digit in 0..60 {
        ans += count(digit) / 2 * Mint::new(1_usize << digit);
    }
...
//         let (mut even, mut odd) = rec(graph, Some(cur), next, digit, cnt);
//         if (weight >> digit) & 1 == 1 {
//             std::mem::swap(&mut even, &mut odd);
//         }
...
//     let sum_even: usize = evens.iter().sum();
//     let sum_odd: usize = odds.iter().sum();

//     for (&even, &odd) in evens.iter().zip(&odds) {
//         *cnt += even * (sum_odd - odd) + odd * (sum_even - even);
//     }
