...
use crate::binary_search::BinarySearchWithI64;

const MAX_ABS_X: i64 = 1_000_000_000;
...
fn main() {
    ...
    let low = apply(-MAX_ABS_X);
    let low_arg = (-MAX_ABS_X..=MAX_ABS_X)
        .binary_search(|x| apply(x) <= low, true)
        .unwrap();
    let high = apply(MAX_ABS_X);

    for &x in &xx {
        let ans = (low + x - low_arg).min(high).max(low);
        println!("{}", ans);
    }
}
