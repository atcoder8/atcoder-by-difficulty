...
fn main() {
    ...
    let s = s.into_iter().map(|c| (c - b'0') as usize).collect_vec();
...
fn solve(mut s: Vec<usize>) -> Option<Mint> {
    let mut aa = vec![];
    let mut nn = vec![];
    ...
    for &c in &s {
        if c == 1 {
            len += 1;
        } else if *aa.last().unwrap() == Mint::new(-1) || len > 0 {
            aa.push(Mint::new(c));
            nn.push(Mint::new(len));
            len = 0;
        } else {
            return None;
        }
    ...
    let mut suffix_one_len = Mint::new(len);
    let mut op_cnt = Mint::new(0);
    for (&a, &n) in aa[1..].iter().zip(&nn).rev() {
        op_cnt += suffix_one_len + 1;
        suffix_one_len = n + (a - 1) * op_cnt;
    }
    op_cnt += suffix_one_len + aa[0];
    ...
    pub fn expected(input: Input) -> Output {
        ...
        while s.len() != 1 {
            ...
            for window in s.windows(2) {
                ...
                t.extend(vec![window[0]; window[1] as usize]);
            }
