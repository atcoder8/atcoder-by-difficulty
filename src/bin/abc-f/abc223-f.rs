...
fn main() {
    ...
    let mut segtree =
        SegmentTree::<S>::from(s.iter().map(|&bracket| S::new(bracket)).collect_vec());
    for (qt, l, r) in queries {
        if qt == 1 {
            s.swap(l, r);
            segtree.set(l, S::new(s[l]));
            segtree.set(r, S::new(s[r]));
        } else {
            let prod = segtree.prod(l..=r);
            let ans = prod.level == 0 && prod.min >= 0;
...
impl S {
    fn new(bracket: char) -> Self {
        let level = if bracket == '(' { 1 } else { -1 };
