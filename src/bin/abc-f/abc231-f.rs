...
fn main() {
    ...
    let mut uo = UniqueOrdering::from(bb.clone());

    let mut ans = 0;
    let mut ft = FenwickTree::<usize>::new(n);
    let sorted_ab = izip!(aa, bb).sorted_by_key(|&(a, b)| (a, Reverse(b)));
    for (cnt, (_, b)) in sorted_ab.into_iter().dedup_with_count() {
        let zip_b = uo.position(&b);
        ft.add(zip_b, cnt);
        ans += cnt * ft.sum(zip_b..);
    }
