...
fn solve() -> bool {
    ...
    hwd.iter_mut().for_each(|x| x.sort_unstable());
    hwd.sort_unstable_by_key(|x| (x[0], Reverse(x[1])));

    let (mut ww, mut dd) = (vec![], vec![]);
    for x in &hwd {
        ww.push(x[1]);
        dd.push(x[2]);
    }

    let (zip_ww, _) = coordinate_compression(&ww);

    let mut segtree: SegmentTree<usize> = SegmentTree::new(n);
    for (&w, &d) in zip_ww.iter().zip(&dd) {
        if segtree.prod(0..w) < d {
            return true;
        }

        segtree.set(w, segtree.get(w).min(d));
    }
