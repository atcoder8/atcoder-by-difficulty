...
fn main() {
    ...
    let mut dp = vec![Mint::new(0); 3001];
    dp[0] = Mint::new(1);

    for i in 0..n {
        let acc: Vec<Mint> = dp.iter().cloned().cumsum().collect();
        let mut next_dp = vec![Mint::new(0); 3001];
    ...
    let ans = dp.iter().fold(Mint::new(0), |acc, &x| acc + x);
    println!("{}", ans.val());
}
