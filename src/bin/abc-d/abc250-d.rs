...
fn main() {
    ...
    let mut acc_prime_num = (0..=10_usize.pow(6))
        .map(|i| sieve.is_prime(i) as usize)
        .collect_vec();
    for i in 0..10_usize.pow(6) {
        acc_prime_num[i + 1] += acc_prime_num[i];
    }

    let mut ans = 0;
    for q in (2_usize..)
        .take_while(|&q| q.pow(3) <= n)
        .filter(|&q| sieve.is_prime(q))
    {
        let max_p = (n / q.pow(3)).min(q - 1);
        ans += acc_prime_num[max_p];
    }
    println!("{}", ans);
}
