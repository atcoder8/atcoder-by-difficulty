...
fn main() {
    ...
    let is_ok = |x: usize| {
        ...
        let count_one = |i: usize, j: usize| {
            let mut ret = acc[i + k - 1][j + k - 1];
            if i > 0 && j > 0 {
                ret += acc[i - 1][j - 1];
            }
            if i > 0 {
    ...
    let max_a = aaa
        .iter()
        .map(|aa| *aa.iter().max().unwrap())
        .max()
        .unwrap();

    let ans = (0..=max_a).binary_search(is_ok, true).unwrap();
    println!("{}", ans);
}
