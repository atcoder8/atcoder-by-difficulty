...
fn main() {
    ...
    memo[0] = Some(Mint::new(1));

    let ans = rec(&mut memo, s);
    println!("{}", ans.val());
}

fn rec(memo: &mut Vec<Option<Mint>>, s: usize) -> Mint {
    if let Some(ret) = memo[s] {
        return ret;
    }

    let ret = (3..=s).fold(Mint::new(0), |acc, i| acc + rec(memo, s - i));
    memo[s] = Some(ret);
